...
app.post("/api/upload-excel", upload.single("file"), (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ success: false, error: "Failed to parse file: " + err.message });
  }
});
...
app.post("/api/campaign/start", async (req, res) => {
  ...
  if (campaignState.isRunning) {
    return res
      .status(400)
      .json({ success: false, error: "Campaign already running" });
  }
...
server.listen(PORT, () => {
  ...
  console.log("Visit your frontend and click Connect WhatsApp to scan QR");
});
